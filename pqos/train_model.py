...
#加载数据
#data = "/Users/quximing/Desktop/Muses_output_100clients.csv"
#data = "/Users/quximing/Desktop/20181024kafkaProduce实验数据整理图片/Muses_output_20181024_kalfaProduce.csv"
data = "./Muses_output.csv"
# df = pd.read_excel(data, header=0, parse_cols="A:E")
df = pd.read_csv(data, header=0, usecols=[0,1,2,3,4,5,6])
...
X = np.asarray(df_update.values[:, 2:7])
y = np.asarray(df_update.values[:, 0])
#判断用pid还是core作为粒度测量IPS
if sum(y) == 0:
    y = np.asarray(df_update.values[:, 1])
...
model_RandomForestRegressor = ensemble.RandomForestRegressor(n_estimators=100)#这里使用100个决策树
####3.6Adaboost回归####
...
# model_XGBRegressor = XGBRegressor(max_depth=6, learning_rate=0.1, n_estimators=100, silent=0, alpha=3, objective='reg:linear',eval_metric='mae')
